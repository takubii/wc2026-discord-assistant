...
6. `DISCORD_WEBHOOK_URL` をWorker Secretに入れる
7. `DISCORD_RESULTS_WEBHOOK_URL` をWorker Secretに入れる
8. 任意で `GEMINI_API_KEY` をWorker Secretに入れる
...
`DISCORD_WEBHOOK_URL` は試合日程チャンネル、`DISCORD_RESULTS_WEBHOOK_URL` は結果チャンネルのWebhook URLを設定します。
