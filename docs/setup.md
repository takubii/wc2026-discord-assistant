...
`DISCORD_WEBHOOK_URL` は試合日程チャンネル、`DISCORD_RESULTS_WEBHOOK_URL` は結果チャンネルのWebhook URLを設定します。`DISCORD_RESULTS_WEBHOOK_URL` が未設定の場合、結果も `DISCORD_WEBHOOK_URL` に投稿されます。
