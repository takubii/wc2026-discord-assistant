...
ワールドカップ2026の翌日分の試合予定を毎日22:00 JSTに、当日の結果とグループ順位を毎日22:10 JSTにDiscordへ投稿します。
...
`.github/workflows/post-worldcup-summary.yml` は毎日22:10 JSTに、今日の結果とGroups A-D / E-H / I-Lの順位を投稿します。
...
代表メンバーはESPNのWorld Cup squad記事から取得し、`data/players.json` にキャッシュします。詳細ポジションと市場価値はTransfermarktから低頻度で補完します。

```powershell
npm run update:players -- --enrich-all
```

現在のキャッシュでは全48チームの代表メンバーを保持し、Transfermarktの国別スカッドページで一致した選手には詳細メインポジションと市場価値を補完しています。Discordコマンド実行時には外部サイトへアクセスせず、キャッシュ済みデータだけを使います。
